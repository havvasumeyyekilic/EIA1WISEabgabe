...
interface Satz { //Interface zur Typisierung spanischer und deutscher Sätze
    frage: string[]; //string ist in der eckigen Klammer
    antwort: string[];
}

var saetze: Satz[] = [ //Array mit Objekten die jeweils ein spanischen und ein deutschen Satz beinhalten
    {
        frage: "Hola, ¡me llamo Pia!".split(" "),  //Trennung der Sätze durch split an jedem Leerzeichen um die Wörter zu separieren
        antwort: "Hallo,Ich heiße Pia!".split(" ")
    },
    {
        frage: "¿Cómo estás?".split(" "),
        antwort: "Wie geht es dir?".split(" ")
    },
    {
        frage: "¿De dónde eres?".split(" "),
        antwort: "Woher kommst du?".split(" ")
    },
    {
        frage: "¿Cuándo es tu cumpleaños?".split(" "),
        antwort: "Wann hast du Geburstag?".split(" ")
    },
    {
        frage: "¿Cuánto cuesta?".split(" "),
        antwort: "Wie viel kostet das?".split(" ") //5 Sätze für eine Kategorie
    }
...
window.addEventListener("load", function (): void {
    ...
    function wortClick(wort: string): void { //wenn ich auf ein Wort klicke gehen die Wörter vom spanischen Satz in den Übersetzungsbereich unter "richtige Wörter"; neue Funktion nach dem anklicken der Wörter wird deklariert
        ...
        if (wort == saetze[satz].frage[richtigeWörter]) { //Wenn das Wort richtig ist...
            ...
            document.getElementById("frage").innerHTML = ""; //das element für den spanisch satz wird geleert; Zugriff auf ID "farge", allgemein: frage zurückgesetzt, wörter werden neu hinzugefügt
            for (var i: number = 0; i < gemischterSpanischerSatz.length; i++) { // das element wird neu befüllt mit den noch verbleibenden spanisch wörtern; Zählschleife, Anfang bei 0, solange i kleiner als die Länge des gemischten Satzes ist, wird i hochgezählt
                ...
                document.getElementById("frage").appendChild(wort); // das wort wird an frage angehängt
            }

            if (richtigeWörter == saetze[satz].frage.length - 1) neuerSatz(); //Wenn die Anzahl der richtigen Wörter im spanischen Satz entspricht ist, dann erscheint der nächste Satz
    ...
    function neuerSatz(): void {
        document.getElementById("antwort").innerHTML = ""; // Felder für Sätze und Wörter werden geleert; es wird auf die ID antwort zugegriffen; dieser wird zurückgesetzt
        ...
        document.getElementById("antwort").innerHTML = saetze[satz].antwort.join(" "); // der deutche Satz, bei dem ich mich gerade befinde wird aus dem Array genommen, zusammengefügt und in das dafür vorgesehene element eingefügt
        var spanisch: string[] = [...saetze[satz].frage]; // der spanische Satz, bei dem ich mich gerade befinde, wird in ein übergangsarray geschrieben
        ...
        for (var i: number = 0; i < spanisch.length; i++) { // es wird durch die einzelnen spansich wörter itteriert
            ...
            document.getElementById("frage").appendChild(wort); // das wort wird in das satz element geschrieben
        }
